...
- **多轮轮次限制**：`round_limit` 限制单次会话最多执行轮数；该限制**仅在自动授权模式生效**——需要用户授权时轮数不受限，会话由用户批准/拒绝命令或停止响应控制结束；**每次授权申请会清零轮数**，中途开启自动授权仍从该授权点获得完整轮数预算
...
- `round_limit`：单次会话最大轮数（默认 20），**仅在自动授权模式生效**；需用户授权时轮数不受限，由用户批准/拒绝命令或停止响应结束会话；每次授权申请清零轮数，中途开启自动授权仍获得完整预算
