...
- **三级授权控制**（`auth_mode`）：`0`=N-Auto（全部命令需授权）、`1`=H-Auto（低风险读取/列出类命令自动执行，高风险新建/删除/编辑/执行等命令需授权）、`2`=F-Auto（全部自动授权）。CLI 用 `/y`/`/n`/`/auth 0|1|2`，Web 用三段式滑动选择器
- **多轮轮次限制**：`round_limit` 限制单次会话最多执行轮数，**与授权模式解耦、任何模式都生效**；自动执行的轮次累加计数，**授权通过时计数清零**（拒绝不清零），故 N-Auto 下每次授权通过后重新获得完整轮数预算
...
- `round_limit`：单次会话最大轮数（默认 20），**与授权模式解耦、任何模式都生效**；自动执行的轮次累加计数，授权通过时清零（拒绝不清零），故 N-Auto 下每次授权通过后重新获得完整预算
...
- **Controls**：主题切换、授权模式（三段式滑动选择：N-Auto / H-Auto / F-Auto）、停止响应、重置会话、关闭 Agent
...
| `/auth 0\|1\|2`            | 查看/切换授权模式（0=N-Auto，1=H-Auto，2=F-Auto）|
...
授权提示时输入 `/y`（允许当前）/ `/n`（拒绝当前）/ `/auth 0|1|2`（切换模式后重新评估当前指令，不再需要授权则放行）/ `/auth`（显示当前模式）。
